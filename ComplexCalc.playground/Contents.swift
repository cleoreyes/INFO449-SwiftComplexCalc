...
calc.add(lhs: 2, rhs: -4) == -2
calc.subtract(lhs: 2, rhs: -4) == 6
...
calc.mathOp(args: [1, 2, -3], beg: 0, op: { $0 + $1 }) == 0
calc.mathOp(args: [], beg: 1, op: { $0 * $1 }) == 0

let pd = ["x": 5, "y": 5]
let pdNoX = ["y": 5]
let pdNoY = ["x": 5]
calc.add(lhs: pd, rhs: pdNoX) == ["x":5, "y": 10]
calc.subtract(lhs: pd, rhs: pdNoX) == ["x":5, "y": 0]
calc.add(lhs: pd, rhs: pdNoY) == ["x":10, "y": 5]
calc.subtract(lhs: pd, rhs: pdNoY) == ["x":0, "y": 5]
